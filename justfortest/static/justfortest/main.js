...
function createDrinkElement(drink) {
    ...
    div.id = drink.id;
    div.title = drink.title;
    div.setAttribute('data-price', drink.price);
    div.onclick = function() { option(this.id, this.title, this.dataset.price); };
    div.className = 'drink'; // 'drink' 클래스 추가
    return div;
}

function displayDrinks(drinks) {
    const container = document.getElementById("추천_음료");
    
    let row = createRow(); // 새로운 row div를 생성하는 함수 호출
    let count = 0; // 현재 row에 추가된 음료 수를 추적

    drinks.forEach(drink => {
        if (count >= 4) { // 현재 row에 4개의 음료가 이미 추가된 경우
            container.appendChild(row); // 현재 row를 container에 추가
            row = createRow(); // 새로운 row div를 생성
            count = 0; // 음료 수 카운터를 초기화
        }
        row.appendChild(createDrinkElement(drink)); // 음료 정보를 현재 row에 추가
        count++; // 음료 수 카운터 증가
    });
...
function createRow() {
    ...
    row.id = "row"; // id 대신 className 사용을 추천 (같은 클래스명을 여러 요소에서 사용 가능)
    return row;
}

const drinks = [
    { id: "ice_아메리카노", title: "coffee", price: 2000 },
